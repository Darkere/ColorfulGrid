...
public class ColoredRelayBlock extends NetworkNodeBlock {

    public ColoredRelayBlock(Properties props) {
        super(props);
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        return !world.isRemote ? NetworkUtils.attemptModify(world, pos, hit.getFace(), player, () -> {
            NetworkHooks.openGui((ServerPlayerEntity)player, new PositionalTileContainerProvider(new TranslationTextComponent("gui.refinedstorage.relay"), (tile, windowId, inventory, p) -> {
                return new RelayContainer((RelayTile) tile, player, windowId);
            }, pos), pos);
    ...
    public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
        return Util.getDrops(state,"relay");
    }
}
